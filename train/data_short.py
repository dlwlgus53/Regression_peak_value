...
class FSIterator:
    ...
    def __next__(self):
        ...
        while(len(bat_seq) < self.batch_size):
            ...
            if(np.count_nonzero(~np.isnan(seq_end)) >= 10 and np.count_nonzero(~np.isnan(seq_end)) < 21):
        ...
        # B x [[E*daylen],[S*daylen],[L*daylen],[H*daylen]]
    ...
    def prepare_data(self, seq):
        ...
        try:
            x_data = np.stack([seq_end_x[:, 1:-PRE_STEP], seq_end_x_delta[:, :-PRE_STEP],
                               seq_start_x[:, 1:-
                                           PRE_STEP], seq_start_x_delta[:, :-PRE_STEP],
                               seq_low_x[:, 1:-
                                         PRE_STEP], seq_low_x_delta[:, :-PRE_STEP],
